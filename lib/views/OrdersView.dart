...
class _OrdersPageState extends State<OrdersPage> {
  ...
  Widget _typeBadge(OrderType type) {
    final (label, color) = switch (type) {
      ...
      OrderType.refund => (context.tr.orderTypeRefund, Colors.purple),
    };
  ...
  Widget _statusBadge(OrderStatus status) {
    final (label, color) = switch (status) {
      OrderStatus.completed => (
        context.tr.orderStatusCompleted,
        const Color(0xFF28A745),
      ),
      OrderStatus.pending => (
        context.tr.orderStatusPending,
        const Color(0xFFFFA500),
      ),
      OrderStatus.canceled => (
        context.tr.orderStatusCanceled,
  ...
  Future<void> _acceptEdit(OrderModel order) async {
    if (order.productId == null) return;
    final proposedExpiry = order.expiryDate ?? order.notes;
    if (proposedExpiry == null || proposedExpiry.startsWith('Invoice: '))
      return;
    ...
    body['expiryDate'] = proposedExpiry;
    ...
    ScaffoldMessenger.of(
      context,
    ).showSnackBar(SnackBar(content: Text(context.tr.editApproved)));
  }
  ...
  Future<void> _rejectEdit(OrderModel order) async {
    ...
    try {
      await OrderService.updateOrderStatus(order.id, OrderStatus.canceled);
    } catch (e) {
      if (!mounted) return;
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(content: Text('$e'), backgroundColor: Colors.red),
    ...
    ScaffoldMessenger.of(
      context,
    ).showSnackBar(SnackBar(content: Text(context.tr.editRejected)));
  }

  void _viewOrderDetails(OrderModel order, bool isDark) {
    final expiry =
        order.expiryDate ??
        (order.notes != null && !order.notes!.startsWith('Invoice: ')
            ? order.notes
            : null);
    final invNum =
        order.invoiceNumber ??
        (order.notes?.startsWith('Invoice: ') == true
            ? order.notes!.substring(9)
            : null);
    showDialog(
      context: context,
      builder: (ctx) => AlertDialog(
        ...
        content: SizedBox(
          ...
          child: Column(
            ...
            children: [
              _detailRow(context.tr.orderId, order.id),
              if (invNum != null) _detailRow(context.tr.invoiceHash, invNum),
              ...
              _detailRow(context.tr.date, _formatDateTime(order.createdAt)),
              if (expiry != null)
                _detailRow(context.tr.expiryDate, _formatRawDate(expiry)),
            ],
          ),
        ),
        actions: [
    ...
    pdf.addPage(
      pw.MultiPage(
        ...
        build: (_) => [
          ...
          pw.Text(
            '${context.tr.generatedPrefix}${_formatDateTime(DateTime.now())}',
          ),
  ...
  String _typeLabel(OrderType type) => switch (type) {
    ...
    OrderType.refund => context.tr.orderTypeRefund,
  };
